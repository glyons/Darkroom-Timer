// Strip Test
void stripValueTest(float t, int mode, int values[],int stops[])
{
...
{
  clearStripLEDs();
  int values[6]; int fstops[6];
  stripValueTest(tensSeconds,stepIdx, values, fstops);        
  for(int n=0;n<=5;n++)
  {
    tm.setLED(n+1,1);
    ...
    {
      FStop=fstops[n];
      timerCountdown(values[n]);//timer with f-stop converted in time + enlargment correction time
      if (FStop < 1000) sprintf(tempString, " %03d%4d", FStop, 0); //fstop format rule 
      if (FStop >= 1000) sprintf(tempString, "%4d%4d", FStop, 0); //fstop format rule
